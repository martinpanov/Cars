export const sellSchema = {
  ...
  phoneNumber: {
    ...
    required: true,
    pattern: /^\+?[0-9\s\-()]{7,15}$/,
    errorMessage: 'Phone number must be between 7 and 15 characters long and can include digits, spaces, dashes, and parentheses.'
  },
  description: {
    type: 'string',
    required: false,
  ...
  images: {
    ...
    minItems: 1,
    maxItems: 12,
    contentMediaType: ['image/jpeg', 'image/png', 'image/webp'],
    maxSize: 5 * 1024 * 1024, // 5MB
    errorMessage: {
      minItems: 'You must upload at least one image.',
      maxItems: 'You can upload 12 images at most.',
      contentMediaType: 'Only JPG, PNG, or WebP images are allowed.',
      maxSize: 'Each image must be less than 5MB.',
