...
dataController.get('/cars', async (req, res) => {
...
dataController.post('/rentcar', hasUser(), async (req, res) => {
  try {
    const car = await rentCar(req.body.carId, req.user._id);
    res.json(car);
  } catch (error) {
    const message = parseError(error);
    res.status(400).json({ message });
...
dataController.put('/cars/:id', hasUser(), upload.array('images', 12), async (req, res) => {
  try {
    const car = await getById(req.params.id);
    ...
    const imagesNames = car.imagesNames.filter(image => !req.body.imagesNames ? image : !req.body.imagesNames.includes(image));
...
dataController.delete('/cars', hasUser(), async (req, res) => {
  try {
